<?
	class WebXtractor_Parser_Html {
		...
		const 	PAGINATION_MIN_LINK_OCCURS = 2;
				...
				foreach($arrImageXPaths as $strImageXPath => $arrData) {
					...
					if (!is_null($imageElements)) {
		  			foreach ($imageElements as $imageElement) {
		  				...
		  				$arrPageImage = array(
		  					'src' => $this->wxUrl->resolveRelative($strSrc),
		  					'href' => $this->wxUrl->resolveRelative($strHref)
		  				);
  				...
  				if (($linkNodeHref = $linkElement->attributes->getNamedItem('href')) &&
		  	      ($strHref = $linkNodeHref->nodeValue)) {
	  				...
	  				$arrPath = explode('/', html_entity_decode(parse_url($strHref, PHP_URL_PATH)));
						//Zend_Registry::get('log')->debug('HTML PARSER: PAGINATOR: ' . join('*', $arrPath));
	  				...
	  				if ($strLastPathItem = array_pop($arrPath)) {
	  					...
	  					// Zend_Registry::get('log')->debug('HTML PARSER: PAGINATOR: ' . $strLastPathItem . ' HAS PAGENUM ' . $intPageNum);
	  					
	  					if ($intPageNum) {
	  						$strStrippedHref = str_replace($strLastPathItem, '', $strHref);
