 	...
 	class WebXtractor_Indexer {
 		...
 		function onWxUrlReadSuccess (WebXtractor_Extractor_Abstract $wxOfferExtractor) {
 			...
 			return function ($wxUrl, $strHtml) use (&$self, $wxOfferExtractor) {
 				Zend_Registry::get('log')->debug("LIB INDEX: READ SUCCESS " . $wxUrl->getUrl());
				...
				Zend_Registry::get('log')->debug('LIB INDEX: ' . count($arrLinksToFollow) . ' NEXT LINKS FOUND');
				...
				foreach($arrLinksToFollow as $strLinkToFollow) {
					...
					Zend_Registry::get('log')->debug('LIB INDEX: NEXT LINK ' . $strLinkToFollow . ' QUEUED TO BOARD ' . $self->wxIndexBoard->id);
		...
		function onWxUrlReadFailed () {
 			...
 			return function ($wxUrl, $strErr) use (&$self) {
 				Zend_Registry::get('log')->debug("LIB INDEX: READ FAILED " . $wxUrl->getUrl() . ': ' . $strErr);
 				$self->wxIndexBoard->setStatus($wxUrl->getUrl(), WebXtractor_Indexer_Board::STATUS_FAILED);
 			};
 		}
 		...
 		function index($mixedWxUrl, WebXtractor_Extractor_Abstract $wxOfferExtractor) {
			...
			foreach ($mixedWxUrl as $wxUrl) {
 			...
 			Zend_Registry::get('log')->debug('LIB INDEX START..: ' . count($arrWxUrlsToIndex) . ' URLS');
 			...
 			Zend_Registry::get('log')->debug('LIB INDEX DONE..: ' . $arrWxUrlsToIndex[0]->getUrl());
 			...
 			Zend_Registry::get('log')->debug('LIB INDEX FINISH..: ' . count($arrWxUrlsToIndex) . ' TO FOLLOW');
